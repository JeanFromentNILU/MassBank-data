...
.scripts/MassBank-web/MassBank-Project/MassBank-lib/bin/Validator -legacy $(cat legacy.list) || haserror=1

echo Validate nonlegacy content
cat non_legacy.list | split -d --additional-suffix .list -C 500K - records
for i in records*.list; do
  .scripts/MassBank-web/MassBank-Project/MassBank-lib/bin/Validator $(cat $i) || haserror=1
done
